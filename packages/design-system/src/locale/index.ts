import type { mageLocale, mageLocaleTranslateFn } from 'mage-design-system/types';
...
let i18nHandler: mageLocaleTranslateFn;
...
export async function use(l: string) {
	try {
		const ndsLang = (await import(`./lang/${l}.ts`)) as { default: mageLocale };
...
export function i18n(fn: mageLocaleTranslateFn) {
