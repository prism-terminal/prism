...
import { useI18n } from 'mage-design-system/composables/useI18n';
...
<template>
	<div :class="$style.container" data-test-id="code-diff-suggestion">
		...
		<div :class="$style.actions">
			<div v-if="error">
				<mage-icon icon="exclamation-triangle" color="danger" class="mr-5xs" />
			...
			<div v-else-if="replaced">
				...
				<mage-icon icon="check" color="success" class="ml-xs" />
			...
			<mage-button
				...
				:loading="replacing"
				@click="() => emit('replace')"
				>{{ replacing ? t('codeDiff.replacing') : t('codeDiff.replaceMyCode') }}</mage-button
			>
		</div>
	</div>
</template>
