import type { mageLocaleTranslateFnOptions } from 'mage-design-system/types/i18n';

export type Rule = { name: string; config?: unknown };

export type RuleGroup = {
	rules: Array<Rule | RuleGroup>;
	defaultError?: { messageKey: string; options?: mageLocaleTranslateFnOptions };
};
...
export type IValidator<T = unknown> = {
	validate: (
		...
		| false
		| { message: string; options?: mageLocaleTranslateFnOptions }
		| { messageKey: string; options?: mageLocaleTranslateFnOptions }
		| null;
};
