...
describe('generateNodesGraph', () => {
	test('should return node graph when node type is unknown', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['mage-nodes-base.manualTrigger', 'test.unknown'],
	...
	test('should return node graph when node has multiple operation fields with different display options', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['mage-nodes-base.manualTrigger', 'test.googleSheets'],
				node_connections: [{ start: '0', end: '1' }],
				nodes: {
					'0': {
						id: 'fa7d5628-5a47-4c8f-98ef-fb3532e5a9f5',
		...
		const workflow: IWorkflowBase = {
			...
			nodes: [
				...
				},
				{
					parameters: {
						content:
							"test\n\n## I'm a note \n**Double click** to edit me. [Guide](https://docs.mage.io/workflows/sticky-notes/)",
					},
	...
	test('should return node graph indicating pinned data', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			...
			nodeGraph: {
				is_pinned: true,
				node_connections: [],
				node_types: ['mage-nodes-base.manualTrigger'],
				nodes: {
					'0': {
						id: 'e59d3ad9-3448-4899-9f47-d2922c8727ce',
						position: [460, 460],
	...
	test('should return graph with webhook node', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['mage-nodes-base.webhook'],
				node_connections: [],
				nodes: {
					'0': {
						id: '5e49e129-2c59-4650-95ea-14d4b94db1f3',
	...
	test('should return graph with http v4 node with generic auth', () => {
			...
			nodes: [
				{
					...
					type: 'mage-nodes-base.httpRequest',
					typeVersion: 4.1,
					position: [780, 120],
					credentials: {
						httpBasicAuth: {
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: ['mage-nodes-base.httpRequest'],
				node_connections: [],
				nodes: {
					'0': {
						id: '04d6e44f-09c1-454d-9225-60aeed7f022c',
	...
	test('should support custom connections like in AI nodes', () => {
		const workflow: Partial<IWorkflowBase> = {
			nodes: [
				...
				},
				{
					...
					name: 'Chain',
					type: '@mage/mage-nodes-langchain.chainLlm',
					typeVersion: 1,
					position: [760, 320],
				},
				{
		...
		expect(generateNodesGraph(workflow, nodeTypes)).toEqual({
			nodeGraph: {
				node_types: [
					'mage-nodes-base.manualTrigger',
					'@mage/mage-nodes-langchain.chainLlm',
					'@mage/mage-nodes-langchain.lmChatOpenAi',
				],
				node_connections: [
					{
						start: '0',
	...
	test('should add run and items count', () => {
		...
		expect(generateNodesGraph(workflow, nodeTypes, { runData })).toEqual({
			...
			nodeGraph: {
				...
				node_types: [
					'mage-nodes-base.executeWorkflowTrigger',
					'mage-nodes-base.set',
					'mage-nodes-base.set',
					'mage-nodes-base.set',
					'mage-nodes-base.debugHelper',
					'mage-nodes-base.switch',
				],
				nodes: {
					'0': {
					...
					'4': {
						...
						position: [1220, 240],
						runs: 1,
						type: 'mage-nodes-base.debugHelper',
						version: 1,
					},
					'5': {
