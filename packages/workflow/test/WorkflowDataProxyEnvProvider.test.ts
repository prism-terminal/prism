...
describe('createEnvProviderState', () => {
	afterEach(() => {
		delete process.env.mage_BLOCK_ENV_ACCESS_IN_NODE;
	});
	...
	it('should block env access when mage_BLOCK_ENV_ACCESS_IN_NODE is set to "true"', () => {
		process.env.mage_BLOCK_ENV_ACCESS_IN_NODE = 'true';
	...
	it('should throw ExpressionError when env access is blocked', () => {
		...
		expect(() => proxy.someEnvVar).toThrowError(
			new ExpressionError('access to env vars denied', {
				causeDetailed:
					'If you need access please contact the administrator to remove the environment variable ‘mage_BLOCK_ENV_ACCESS_IN_NODE‘',
				runIndex: 1,
				itemIndex: 1,
			}),
		);
