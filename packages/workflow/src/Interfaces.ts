...
import type { LogScope } from '@mage/config';
...
export namespace DeclarativeRestApiSettings {
	...
	export type ResultOptions = {
		...
		requestOperations?: ImageRequestOperations;
	};
}
...
export type IExecuteResponsePromiseData = IDataObject | ImageHttpFullResponse;
...
export type ImageHttpResponse = IDataObject | Buffer | GenericValue | GenericValue[] | null;

export interface ImageHttpFullResponse {
	body: ImageHttpResponse | Readable;
...
export interface ImageRequestOperations {
	pagination?:
		| ImageRequestOperationPaginationGeneric
		| ImageRequestOperationPaginationOffset
		| ((
...
export interface ImageRequestOperationPaginationBase {
	type: string;
	properties: {
		[key: string]: unknown;
	};
...
export interface ImageRequestOperationPaginationGeneric extends ImageRequestOperationPaginationBase {
	type: 'generic';
	properties: {
		continue: boolean | string;
		request: IRequestOptionsSimplifiedAuth;
...
export interface ImageRequestOperationPaginationOffset extends ImageRequestOperationPaginationBase {
	type: 'offset';
	properties: {
...
namespace ExecuteFunctions {
	...
	export type GetNodeParameterFn = {
		// @TECH_DEBT: Refactor to remove this barely used overload - mage-5632
...
export interface ILoadOptions {
	routing?: {
		operations?: ImageRequestOperations;
...
export interface INodePropertyRouting {
	operations?: ImageRequestOperations; // Should be changed, does not sound right
	output?: INodeRequestOutput;
	request?: DeclarativeRestApiSettings.HttpRequestOptions;
...
export type NodeDefaults = Partial<{
	/**
	 * @deprecated Use {@link INodeTypeBaseDescription.iconColor|iconColor} instead. `iconColor` supports dark mode and uses preset colors from mage's design system.
	 */
	color: string;
	name: string;
}>;
...
export interface INodeGraphItem {
	...
	agent?: string; //@mage/mage-nodes-langchain.agent
	...
	workflow_id?: string; //@mage/mage-nodes-langchain.toolWorkflow and mage-nodes-base.executeWorkflow
	runs?: number;
	items_total?: number;
}
...
export type mageAIProviderType = 'openai' | 'unknown';
...
export type IPersonalizationSurveyAnswersV4 = {
	...
	personalization_survey_mage_version: string;
