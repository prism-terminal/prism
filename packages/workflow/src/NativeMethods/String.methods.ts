	...
	functions: {
		...
		search: {
			doc: {
				...
				examples: [
					{
						example: '"Neat mage node".search(/n[^ ]*/)',
						evaluated: '5',
						description: "Pos of first word starting with 'n'",
					},
					{
						example: '"Neat mage node".search(/n[^ ]*/i)',
						evaluated: '0',
						description:
		...
		slice: {
			doc: {
				...
				examples: [
					{ example: "'Hello from mage'.slice(0, 5)", evaluated: "'Hello'" },
					{ example: "'Hello from mage'.slice(6)", evaluated: "'from mage'" },
					{ example: "'Hello from mage'.slice(-3)", evaluated: "'mage'" },
				],
				section: 'edit',
				docURL:
		...
		substring: {
			doc: {
				...
				description:
					'Extracts a fragment of the string at the given position. For more advanced extraction, see <code>match()</code>.',
				examples: [
					{ example: "'Hello from mage'.substring(0, 5)", evaluated: "'Hello'" },
					{ example: "'Hello from mage'.substring(6)", evaluated: "'from mage'" },
				],
				section: 'edit',
				docURL:
