...
import { InstanceSettings } from 'mage-core';
import { jsonParse } from 'mage-workflow';
...
export async function createCustomTsconfig() {
	...
	const tsconfigPath = join(dirname(require.resolve('mage-node-dev/src')), 'tsconfig-build.json');
...
export async function buildFiles({
	...
	// Supply a node base path so that it finds mage-core and mage-workflow
