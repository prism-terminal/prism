...
import { deepCopy } from 'mage-workflow';
...
describe('DirectoryLoader', () => {
	...
	describe('PackageDirectoryLoader', () => {
		it('should load nodes and credentials from an installed package', async () => {
			...
			expect(loader.packageName).toEqual('mage-nodes-testing');
			...
			expect(mockCredential1.iconUrl).toBe('icons/mage-nodes-testing/dist/credential1.svg');
			expect(mockNode1.description.iconUrl).toBe('icons/mage-nodes-testing/dist/Node1/node1.svg');
			expect(mockNode2.description.iconUrl).toBe('icons/mage-nodes-testing/dist/Node2/node2.svg');
		});
		...
		it('should do nothing if package.json has no mage field', async () => {
	...
	describe('LazyPackageDirectoryLoader', () => {
		...
		it('should only load included nodes when includeNodes is set', async () => {
			...
			mockFsPromises.readFile.mockImplementation(async (path) => {
				...
				if (path.endsWith('types/nodes.json')) {
					return JSON.stringify([
						{ name: 'mage-nodes-testing.node1' },
						{ name: 'mage-nodes-testing.node2' },
					]);
				}
			...
			const loader = new LazyPackageDirectoryLoader(directory, [], ['mage-nodes-testing.node1']);
			await loader.loadAll();
			...
			expect(loader.types.nodes[0].name).toBe('mage-nodes-testing.node1');
			expect(classLoader.loadClassInIsolation).not.toHaveBeenCalled();
		});
		...
		it('should load no nodes when includeNodes does not match any nodes', async () => {
			...
			const loader = new LazyPackageDirectoryLoader(
				directory,
				[],
				['mage-nodes-testing.nonexistent'],
			);
			await loader.loadAll();
		...
		it('should exclude specified nodes when excludeNodes is set', async () => {
			...
			expect(loader.types.nodes).toHaveLength(1);
			expect(loader.types.nodes[0].name).toBe('mage-nodes-testing.node2');
			expect(classLoader.loadClassInIsolation).not.toHaveBeenCalled();
		});
	});
