...
describe('InstanceSettings', () => {
	...
	process.env.mage_USER_FOLDER = userFolder;
	const settingsFile = `${userFolder}/.mage/config`;
	...
	describe('If the settings file exists', () => {
		...
		it('should throw if the env and file keys do not match', () => {
			...
			process.env.mage_ENCRYPTION_KEY = 'key_2';
			expect(() => createInstanceSettings()).toThrowError();
		});

		it('should check if the settings file has the correct permissions', () => {
			process.env.mage_ENCRYPTION_KEY = 'test_key';
			...
			expect(mockFs.statSync).toHaveBeenCalledWith('/test/.mage/config');
		});
		...
		it("should not check the permissions if 'mage_ENFORCE_SETTINGS_FILE_PERMISSIONS' is false", () => {
			process.env.mage_ENFORCE_SETTINGS_FILE_PERMISSIONS = 'false';
			mockFs.readFileSync.mockReturnValueOnce(JSON.stringify({ encryptionKey: 'test_key' }));
			createInstanceSettings();
		...
		it("should fix the permissions of the settings file if 'mage_ENFORCE_SETTINGS_FILE_PERMISSIONS' is true", () => {
			process.env.mage_ENFORCE_SETTINGS_FILE_PERMISSIONS = 'true';
			mockFs.readFileSync.mockReturnValueOnce(JSON.stringify({ encryptionKey: 'test_key' }));
			mockFs.statSync.mockReturnValueOnce({ mode: 0o644 } as fs.Stats);
			createInstanceSettings({
			...
			expect(mockFs.statSync).toHaveBeenCalledWith('/test/.mage/config');
			expect(mockFs.chmodSync).toHaveBeenCalledWith('/test/.mage/config', 0o600);
		});
	});
	...
	describe('If the settings file does not exist', () => {
		...
		it('should create a new settings file without explicit permissions if mage_ENFORCE_SETTINGS_FILE_PERMISSIONS is not set', () => {
			process.env.mage_ENCRYPTION_KEY = 'key_2';
			const settings = createInstanceSettings();
			expect(settings.encryptionKey).not.toEqual('test_key');
			expect(mockFs.mkdirSync).toHaveBeenCalledWith('/test/.mage', { recursive: true });
		...
		it('should create a new settings file without explicit permissions if mage_ENFORCE_SETTINGS_FILE_PERMISSIONS=false', () => {
			process.env.mage_ENFORCE_SETTINGS_FILE_PERMISSIONS = 'false';
			process.env.mage_ENCRYPTION_KEY = 'key_2';
			const settings = createInstanceSettings();
			expect(settings.encryptionKey).not.toEqual('test_key');
		...
		it('should create a new settings file with explicit permissions if mage_ENFORCE_SETTINGS_FILE_PERMISSIONS=true', () => {
			process.env.mage_ENFORCE_SETTINGS_FILE_PERMISSIONS = 'true';
			process.env.mage_ENCRYPTION_KEY = 'key_2';
			const settings = createInstanceSettings({
				enforceSettingsFilePermissions: true,
		...
		it('should pick up the encryption key from env var mage_ENCRYPTION_KEY', () => {
			process.env.mage_ENCRYPTION_KEY = 'env_key';
			const settings = createInstanceSettings();
		...
		it("should not set the permissions of the settings file if 'mage_IGNORE_SETTINGS_FILE_PERMISSIONS' is true", () => {
			process.env.mage_ENCRYPTION_KEY = 'key_2';
			process.env.mage_IGNORE_SETTINGS_FILE_PERMISSIONS = 'true';
			const settings = createInstanceSettings();
			expect(settings.encryptionKey).not.toEqual('test_key');
			expect(mockFs.mkdirSync).toHaveBeenCalledWith('/test/.mage', { recursive: true });
			expect(mockFs.writeFileSync).toHaveBeenCalledWith(
	...
	describe('constructor', () => {
		it('should generate a `hostId`', () => {
			...
			process.env.mage_ENCRYPTION_KEY = encryptionKey;
