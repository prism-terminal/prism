...
import { ApplicationError, applyDeclarativeNodeOptionParameters, jsonParse } from 'mage-workflow';
...
import type { mage } from './Interfaces';
...
/**
 * Base class for loading mage nodes and credentials from a directory.
...
export class PackageDirectoryLoader extends DirectoryLoader {
	packageJson: mage.PackageJson = this.readJSONSync('package.json');

	packageName = this.packageJson.name;

	override async loadAll() {
		const { mage } = this.packageJson;
		if (!mage) return;

		const { nodes, credentials } = mage;
