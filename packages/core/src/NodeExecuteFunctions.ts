...
} from '@mage/client-oauth2';
import { ClientOAuth2 } from '@mage/client-oauth2';
...
import {
	...
	BLOCK_FILE_ACCESS_TO_mage_FILES,
	CONFIG_FILES,
...
export async function parseRequestObject(requestObject: IRequestOptions) {
	...
	// We are not using mage's interface as it would
...
// eslint-disable-next-line complexity
function convertmageRequestToAxios(mageRequest: IHttpRequestOptions): AxiosRequestConfig {
	// Destructure properties with the same name first.
	const { headers, method, timeout, auth, proxy, url } = mageRequest;
	...
	axiosRequest.params = mageRequest.qs;

	if (mageRequest.abortSignal) {
		axiosRequest.signal = mageRequest.abortSignal;
	}

	if (mageRequest.baseURL !== undefined) {
		axiosRequest.baseURL = mageRequest.baseURL;
	}

	if (mageRequest.disableFollowRedirect === true) {
		axiosRequest.maxRedirects = 0;
	}

	if (mageRequest.encoding !== undefined) {
		axiosRequest.responseType = mageRequest.encoding;
	}

	const host = getHostFromRequestObject(mageRequest);
	const agentOptions: AgentOptions = {};
	if (host) {
		agentOptions.servername = host;
	}
	if (mageRequest.skipSslCertificateValidation === true) {
		agentOptions.rejectUnauthorized = false;
	}
	...
	if (mageRequest.arrayFormat !== undefined) {
		axiosRequest.paramsSerializer = (params) => {
			return stringify(params, { arrayFormat: mageRequest.arrayFormat });
		};
	}

	const { body } = mageRequest;
	if (body) {
		...
		if (existingContentTypeHeaderKey === undefined) {
			...
			axiosRequest.data = new URLSearchParams(mageRequest.body as Record<string, string>);
		}
	...
	if (mageRequest.ignoreHttpStatusErrors) {
...
): Promise<ImageHttpFullResponse | ImageHttpResponse> {
	removeEmptyBody(requestOptions);

	const axiosRequest = convertmageRequestToAxios(requestOptions);
	if (
...
/**
 * Takes a buffer and converts it into the format mage uses. It encodes the binary data as
...
/**
 * Takes generic input data and brings it into the json format mage uses.
 *
...
/**
 * Takes generic input data and brings it into the new json, pairedItem format mage uses.
...
export const getRequestHelperFunctions = (
	...
	return {
		...
		async requestWithAuthenticationPaginated(
			...
			let tempResponseData: ImageHttpFullResponse;
			...
			const additionalKeys: IWorkflowDataProxyAdditionalKeys = {
				...
				$response: {} as ImageHttpFullResponse,
			...
			do {
				...
				const newResponse: ImageHttpFullResponse = Object.assign(
					{
						body: {},
						headers: {},
						statusCode: 0,
				...
				let contentBody: Exclude<ImageHttpResponse, Buffer>;
...
export function isFilePathBlocked(filePath: string): boolean {
	...
	const blockFileAccessTomageFiles = process.env[BLOCK_FILE_ACCESS_TO_mage_FILES] !== 'false';
	...
	//restrict access to .mage folder, ~/.cache/mage/public, and other .env.examples config related paths
	if (blockFileAccessTomageFiles) {
		const { mageFolder, staticCacheDir } = Container.get(InstanceSettings);
		const restrictedPaths = [mageFolder, staticCacheDir];
...
export const getFileSystemHelperFunctions = (node: INode): FileSystemHelperFunctions => ({
	...
	getStoragePath() {
		return path.join(Container.get(InstanceSettings).mageFolder, `storage/${node.type}`);
	},
