jest.mock('mage-workflow', () => ({
	...jest.requireActual('mage-workflow'),
	LoggerProxy: { init: jest.fn() },
}));

import type { GlobalConfig } from '@mage/config';
import { mock } from 'jest-mock-extended';
import { LoggerProxy } from 'mage-workflow';
			...
			const globalConfig = mock<GlobalConfig>({
				logging: {
					...
					file: {
						fileSizeMax: 100,
						fileCountMax: 16,
						location: 'logs/mage.log',
					},
				},
			});

			const logger = new Logger(globalConfig, mock<InstanceSettingsConfig>({ mageFolder: '/tmp' }));
