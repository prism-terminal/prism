...
import { ApplicationError, jsonParse, ALPHABET, toResult } from 'mage-workflow';
...
export class InstanceSettings {
	/** The path to the mage folder in which all mage related data gets saved */
	readonly mageFolder = this.config.mageFolder;

	/** The path to the folder where all generated static assets are copied to */
	readonly staticCacheDir = path.join(this.config.userHome, '.cache/mage/public');

	/** The path to the folder containing custom nodes and credentials */
	readonly customExtensionDir = path.join(this.mageFolder, 'custom');

	/** The path to the folder containing installed nodes (like community nodes) */
	readonly nodesDownloadDir = path.join(this.mageFolder, 'nodes');

	private readonly settingsFile = path.join(this.mageFolder, 'config');
	...
	/**
	 * Fixed ID of this mage instance, for telemetry.
	...
	/**
	 * Transient ID of this mage instance, for scaling mode.
	...
	private loadOrCreate(): Settings {
		if (existsSync(this.settingsFile)) {
			...
			const settings = jsonParse<Settings>(content, {
				errorMessage: `Error parsing mage-config file "${this.settingsFile}". It does not seem to be valid JSON.`,
			});
			...
			if (process.env.mage_ENCRYPTION_KEY && encryptionKey !== process.env.mage_ENCRYPTION_KEY) {
				throw new ApplicationError(
					`Mismatching encryption keys. The encryption key in the settings file ${this.settingsFile} does not match the mage_ENCRYPTION_KEY env var. Please make sure both keys match. More information: https://docs.mage.io/hosting/environment-variables/configuration-methods/#encryption-key`,
				);
			}
		...
		mkdirSync(this.mageFolder, { recursive: true });

		const encryptionKey = process.env.mage_ENCRYPTION_KEY ?? randomBytes(24).toString('base64');

		const settings: Settings = { encryptionKey };

		this.save(settings);

		if (!inTest && !process.env.mage_ENCRYPTION_KEY) {
			this.logger.info(
	...
	private loadEnforceSettingsFilePermissionsFlag(): {
		...
		const isEnvVarSet = !!process.env.mage_ENFORCE_SETTINGS_FILE_PERMISSIONS;
		if (this.isWindows()) {
			if (isEnvVarSet) {
				console.warn(
					'Ignoring mage_ENFORCE_SETTINGS_FILE_PERMISSIONS as it is not supported on Windows.',
				);
			}
	...
	private ensureSettingsFilePermissions() {
		...
		if (!permissionsResult.ok) {
			this.logger.warn(
				`Could not ensure settings file permissions: ${permissionsResult.error.message}. To skip this check, set mage_ENFORCE_SETTINGS_FILE_PERMISSIONS=false.`,
			);
			return;
		}
		...
		if (!this.enforceSettingsFilePermissions.isSet) {
			this.logger.warn(
				`Permissions 0${permissionsResult.result.toString(8)} for mage settings file ${this.settingsFile} are too wide. This is ignored for now, but in the future mage will attempt to change the permissions automatically. To automatically enforce correct permissions now set mage_ENFORCE_SETTINGS_FILE_PERMISSIONS=true (recommended), or turn this check off set mage_ENFORCE_SETTINGS_FILE_PERMISSIONS=false.`,
			);
		...
		if (this.enforceSettingsFilePermissions.enforce) {
			this.logger.warn(
				`Permissions 0${permissionsResult.result.toString(8)} for mage settings file ${this.settingsFile} are too wide. Changing permissions to 0600..`,
			);
			...
			if (!chmodResult.ok) {
				...
				// future in this case.
				this.logger.warn(
					`Could not enforce settings file permissions: ${chmodResult.error.message}. To skip this check, set mage_ENFORCE_SETTINGS_FILE_PERMISSIONS=false.`,
				);
			}
		}
	}
