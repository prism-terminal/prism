...
import type { FilterOperatorType } from 'mage-workflow';
...
<template>
	...
	>
		<template v-if="selectedGroupIcon" #prefix>
			<mage-icon :class="$style.icon" :icon="selectedGroupIcon" color="text-light" size="small" />
		</template>
					...
					<template #reference>
						...
						>
							<div :class="$style.groupTitle">
								<mage-icon v-if="group.icon" :icon="group.icon" :class="$style.icon" size="small" />
								<span>{{ i18n.baseText(group.name) }}</span>
							</div>
							<mage-icon icon="chevron-right" color="text-light" size="xsmall" />
						</div>
					</template>
					<div>
						<mage-option
