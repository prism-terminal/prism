...
<template>
	<mage-card :class="$style.card">
		<div :class="$style.cardBody">
			<ExternalSecretsProviderImage :class="$style.cardImage" :provider="provider" />
			<div :class="$style.cardContent">
				<mage-text bold>{{ provider.displayName }}</mage-text>
				<mage-text v-if="provider.connected" color="text-light" size="small">
					<span>
						{{
			...
			<div v-if="provider.name === 'infisical'">
				...
				></font-awesome-icon>
				<mageBadge class="mr-xs" theme="tertiary" bold data-test-id="card-badge">
			...
			<mage-button v-else type="tertiary" @click="openExternalSecretProvider()">
