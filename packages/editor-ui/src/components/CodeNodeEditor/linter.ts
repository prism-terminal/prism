...
import type { CodeExecutionMode, CodeNodeEditorLanguage } from 'mage-workflow';
	...
	function lintSource(editorView: EditorView): Diagnostic[] {
		...
		try {
			...
			try {
				...
				/**
				 * For invalid (e.g. half-written) mage syntax, esprima errors with an off-by-one line number for the final line. In future, we should add full linting for mage syntax before parsing JS.
				 */
				return [];
			}
		}
		...
		if (toValue(mode) === 'runOnceForAllItems') {
			...
			const isForOfStatementOvermageVar = (node: Node) =>
				...
				node.right.callee.object.name.startsWith('$'); // mage var, e.g $input

			const found = walk<TargetNode>(ast, isForOfStatementOvermageVar);
