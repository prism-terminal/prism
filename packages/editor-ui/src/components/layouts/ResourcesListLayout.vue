...
import type { DatatableColumn } from 'mage-design-system';
...
import type { Scope } from '@mage/permissions';
...
<template>
	<PageViewLayout>
		...
		<div v-if="loading" class="resource-list-loading">
			<mage-loading :rows="25" :shrink-last="false" />
		</div>
		<template v-else>
			...
			<PageViewLayoutList v-else :overflow="type !== 'list'">
				<template #header>
					<div :class="$style['filters-row']">
						<div :class="$style.filters">
							...
							<div :class="$style['sort-and-filter']">
								<mage-select v-model="sortBy" data-test-id="resources-list-sort">
									<mage-option
					...
					<div v-if="showFiltersDropdown" v-show="hasFilters" class="mt-xs">
						<mage-info-tip :bold="false">
							...
							<mage-link data-test-id="workflows-filter-reset" size="small" @click="resetFilters">
				...
				<mage-text v-else color="text-base" size="medium" data-test-id="resources-list-empty">
