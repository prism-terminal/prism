...
import type { WorkerStatus } from '@mage/api-types';
...
<template>
	<mage-card v-if="worker" :class="$style.cardLink">
		<template #header>
			<mage-heading
				tag="h2"
				bold
		...
		<div :class="$style.cardDescription">
			<mage-text color="text-light" size="small" :class="$style.container">
				<span
					>{{ i18n.baseText('workerList.item.lastUpdated') }} {{ secondsSinceLastUpdateString }}s
					ago | mage-Version: {{ worker.version }} | Architecture: {{ worker.arch }} (
