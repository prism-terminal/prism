...
import { mageOption, mageSelect } from 'mage-design-system';
...
const selectRef = ref<InstanceType<typeof mageSelect>>();
const tagRefs = ref<Array<InstanceType<typeof mageOption>>>();
const createRef = ref<InstanceType<typeof mageOption>>();
...
<template>
	<div ref="container" :class="{ 'tags-container': true, focused }" @keydown.stop>
		...
		>
			...
			<mageOption v-else-if="options.length === 0" value="message" disabled>
			...
			<mageOption :key="MANAGE_KEY" :value="MANAGE_KEY" class="ops manage-tags">
