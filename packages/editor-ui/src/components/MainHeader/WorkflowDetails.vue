	...
	<div :class="$style.container">
		...
		<PushConnectionTracker class="actions">
			...
			<EnterpriseEdition :features="[EnterpriseEditionFeature.Sharing]">
				...
				<template #fallback>
					<mageTooltip>
						<mageButton type="secondary" :class="['mr-2xs', $style.disabledShareButton]">
							{{ i18n.baseText('workflowDetails.share') }}
						</mageButton>
						<template #content>
							<i18n-t
			...
			<div :class="[$style.workflowMenuContainer, $style.group]">
				...
				/>
				<mageTooltip :visible="isNodeViewDiscoveryTooltipVisible">
					<mageActionDropdown
					...
					<template #content>
						<div class="mb-4xs">
							<mageBadge>{{ i18n.baseText('menuActions.badge.beta') }}</mageBadge>
						</div>
						<p>{{ i18n.baseText('menuActions.nodeViewDiscovery.tooltip') }}</p>
						<mageText color="text-light" size="small">
