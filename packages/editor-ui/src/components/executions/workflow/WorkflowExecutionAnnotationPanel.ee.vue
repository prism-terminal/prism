...
import type { AnnotationVote, ExecutionSummary } from 'mage-workflow';
	...
	>
		...
		<div :class="$style.section">
			<div :class="$style.heading">
				<mage-heading tag="h3" size="small" color="text-dark">
			...
			>
				...
				>
					<mage-text :class="$style.key" size="small" color="text-base">
						{{ attr }}
					</mage-text>
					<mage-text :class="$style.value" size="small" color="text-base">
						{{ activeExecution?.customData[attr] }}
					</mage-text>
				</div>
			</div>
			<div v-else :class="$style.noResultsContainer" data-test-id="execution-annotation-data-empty">
				<mage-text color="text-base" size="small" align="center">
					<span v-mage-html="i18n.baseText('executionAnnotationView.data.notFound')" />
				</mage-text>
			</div>
		</div>
	</div>
