...
const onDownloadClick = () => {
	const filename = 'mage-recovery-codes.txt';
		...
		<template #content>
			<div v-if="!showRecoveryCodes" :class="[$style.container, $style.modalContent]">
				<div :class="$style.textContainer">
					<mage-text size="large" color="text-dark" :bold="true">{{
						i18n.baseText('mfa.setup.step1.instruction1.title')
					}}</mage-text>
				</div>
				<div>
					<mage-text size="medium" :bold="false">
			...
			<div v-else :class="$style.container">
				...
				<div :class="$style.recoveryCodesContainer">
					<div v-for="recoveryCode in recoveryCodes" :key="recoveryCode">
						<mage-text size="medium">{{ recoveryCode }}</mage-text>
					</div>
				</div>
				<mage-info-tip :bold="false" :class="$style['edit-mode-footer-infotip']">
					<i18nn-t keypath="mfa.setup.step2.infobox.description" tag="span">
						<template #part1>
							{{ i18n.baseText('mfa.setup.step2.infobox.description.part1') }}
						</template>
						<template #part2>
							<mage-text size="small" :bold="true" :class="$style.loseAccessText">
