...
import type { IMenuItem } from 'mage-design-system';
...
const sidebarMenuItems = computed<IMenuItem[]>(() => {
	const menuItems: IMenuItem[] = [
		...
		},
		{
			id: 'settings-api',
			icon: 'plug',
			label: i18n.baseText('settings.mageapi'),
			position: 'top',
...
<template>
	<div :class="$style.container">
		<mage-menu :items="sidebarMenuItems">
			<template #header>
				<div :class="$style.returnButton" data-test-id="settings-back" @click="emit('return')">
					<i class="mr-xs">
						<font-awesome-icon icon="arrow-left" />
					</i>
					<mage-heading size="large" :bold="true">{{ i18n.baseText('settings') }}</mage-heading>
				</div>
			</template>
			<template #menuSuffix>
				<div :class="$style.versionContainer">
					<mage-link size="small" @click="uiStore.openModal(ABOUT_MODAL_KEY)">
