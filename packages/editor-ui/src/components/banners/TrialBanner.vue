...
<template>
	<BaseBanner name="TRIAL" theme="custom">
			...
			<div :class="$style.content">
				...
				<div :class="$style.usageCounter">
					...
					<div :class="$style.executionsCountSection">
						<mageText size="xsmall" :color="trialHasExecutionsLeft ? 'text-dark' : 'danger'">
							{{ currentExecutions }}/{{ maxExecutions }} </mageText
						>&nbsp;<mageText
							size="xsmall"
							:color="trialHasExecutionsLeft ? 'text-dark' : 'danger'"
							>{{ locale.baseText('executionUsage.label.executions') }}</mageText
						>
					</div>
				</div>
			</div>
		...
		<template #trailingContent>
			<mage-button type="success" icon="gem" size="small" @click="onUpdatePlanClick">{{
