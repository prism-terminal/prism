...
import type { mageBanners } from '@/Interface';
...
// https://www.notion.so/mage/Banner-stack-60948c4167c743718fde80d6745258d5
export const mage_BANNERS: mageBanners = {
...
const currentlyShownBanner = computed(() => {
	...
	let banner = mage_BANNERS[uiStore.bannerStack[0]];
	uiStore.bannerStack.forEach((bannerName, index) => {
		if (index === 0) return;
		const bannerToCompare = mage_BANNERS[bannerName];
