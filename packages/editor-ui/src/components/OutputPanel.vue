...
import { mageRadioButtons, mageText } from 'mage-design-system';
...
<template>
	...
	>
		...
		<template #node-not-run>
			<mageText v-if="workflowRunning && !isTriggerNode" data-test-id="ndv-output-waiting">{{
				i18n.baseText('ndv.output.waitingToRun')
			}}</mageText>
			<mageText v-if="!workflowRunning" data-test-id="ndv-output-run-node-hint">
				...
				<template v-else>
					...
					<span v-if="canPinData" @click="insertTestData">
						...
						<mageText tag="a" size="medium" color="primary">
		...
		<template #node-waiting>
			...
			<mageText v-mage-html="waitingNodeTooltip(node)"></mageText>
		</template>

		<template #no-output-data>
			<mageText :bold="true" color="text-dark" size="large">{{
