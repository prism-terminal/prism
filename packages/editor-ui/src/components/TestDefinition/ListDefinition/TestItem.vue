...
import mageIconButton from 'mage-design-system/components/mageIconButton';
	...
	<div :class="$style.testItem" @click="$emit('view-details', test.id)">
		<div :class="$style.testInfo">
			<div :class="$style.testName">
				{{ test.name }}
				<mage-tag v-if="test.tagName" :text="test.tagName" />
			</div>
			<div :class="$style.testCases">
				{{ locale.baseText('testDefinition.list.testCases', { adjustToNumber: test.testCases }) }}
				<mage-loading v-if="!test.execution.lastRun" :loading="true" :rows="1" />
				<span v-else>{{
		...
		<div :class="$style.actions">
			<mage-tooltip v-for="action in actions" :key="action.icon" placement="top" :show-after="1000">
				<template #content>
					{{ action.tooltip }}
				</template>
				<component
