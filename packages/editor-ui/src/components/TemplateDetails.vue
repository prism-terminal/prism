...
<template>
	<div>
		<mage-loading :loading="loading" :rows="5" variant="p" />
		...
		>
			<mage-tags :tags="template.categories" @click:tag="redirectToCategory" />
		</TemplateDetailsBlock>
		...
		>
			<div :class="$style.text">
				<mage-text v-if="isTemplatesWorkflow(template)" size="small" color="text-base">
					{{ i18n.baseText('template.details.created') }}
					<TimeAgo :date="template.createdAt" />
					{{ i18n.baseText('template.details.by') }}
					{{ template.user ? template.user.username : 'mage team' }}
				</mage-text>
			</div>
			<div :class="$style.text">
				<mage-text
