...
import type { PublicInstalledPackage } from 'mage-workflow';
...
<template>
	<div :class="$style.cardContainer" data-test-id="community-package-card">
		<div v-if="loading" :class="$style.cardSkeleton">
			<mage-loading :class="$style.loader" variant="p" :rows="1" />
			<mage-loading :class="$style.loader" variant="p" :rows="1" />
		</div>
		<div v-else-if="communityPackage" :class="$style.packageCard">
			<div :class="$style.cardInfoContainer">
				<div :class="$style.cardTitle">
					<mage-text :bold="true" size="large">{{ communityPackage.packageName }}</mage-text>
				</div>
				<div :class="$style.cardSubtitle">
					<mage-text :bold="true" size="small" color="text-light">
						{{
			...
			<div :class="$style.cardControlsContainer">
				<mage-text :bold="true" size="large" color="text-light">
					v{{ communityPackage.installedVersion }}
				</mage-text>
				<mage-tooltip v-if="communityPackage.failedLoading === true" placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.failedToLoad.tooltip') }}
						</div>
					</template>
					<mage-icon icon="exclamation-triangle" color="danger" size="large" />
				</mage-tooltip>
				<mage-tooltip v-else-if="communityPackage.updateAvailable" placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.updateAvailable.tooltip') }}
						</div>
					</template>
					<mage-button outline label="Update" @click="onUpdateClick" />
				</mage-tooltip>
				<mage-tooltip v-else placement="top">
					<template #content>
						<div>
							{{ i18n.baseText('settings.communityNodes.upToDate.tooltip') }}
						</div>
					</template>
					<mage-icon icon="check-circle" color="text-light" size="large" />
				</mage-tooltip>
				<div :class="$style.cardActions">
					<mage-action-toggle :actions="packageActions" @action="onAction"></mage-action-toggle>
				</div>
			</div>
		</div>
	</div>
