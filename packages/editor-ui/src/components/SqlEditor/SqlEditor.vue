...
import { mageCompletionSources } from '@/plugins/codemirror/completions/addCompletions';
...
import { mageAutocompletion } from '@/plugins/codemirror/mageLang';
...
const extensions = computed(() => {
	...
	function sqlWithmageLanguageSupport() {
		return new LanguageSupport(dialect.language, [
			dialect.language.data.of({
				autocomplete: ifNotIn(['Resolvable'], keywordCompletionSource(dialect, true)),
			}),
			mageCompletionSources().map((source) => dialect.language.data.of(source)),
		]);
	}

	const baseExtensions = [
		sqlWithmageLanguageSupport(),
