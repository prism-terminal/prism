		...
		<template #content>
			<div :class="[$style.descriptionContainer, 'p-s']">
				<div>
					...
					{{ ' ' }}
					<mage-link :to="COMMUNITY_NODES_INSTALLATION_DOCS_URL" @click="onMoreInfoTopClick">
			...
			<div :class="[$style.formContainer, 'mt-m']">
				...
				>
					<mage-text>
						{{ i18n.baseText('settings.communityNodes.installModal.checkbox.label') }} </mage-text
					><br />
					<mage-link :to="COMMUNITY_NODES_RISKS_DOCS_URL" @click="onLearnMoreLinkClick">{{
						i18n.baseText('generic.moreInfo')
					}}</mage-link>
				</el-checkbox>
			</div>
