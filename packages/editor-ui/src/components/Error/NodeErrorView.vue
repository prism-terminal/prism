...
function copyErrorDetails() {
	...
	//add mage details
	const mageDetails: IDataObject = {};

	if (error.node) {
		mageDetails.nodeName = error.node.name;
		mageDetails.nodeType = error.node.type;
		mageDetails.nodeVersion = error.node.typeVersion;

		if (error.node?.parameters?.resource) {
			mageDetails.resource = error.node.parameters.resource;
		}
		if (error?.node?.parameters?.operation) {
			mageDetails.operation = error.node.parameters.operation;
		}
	}

	if (error.context) {
		if (error.context.itemIndex !== undefined) {
			mageDetails.itemIndex = error.context.itemIndex;
		}

		if (error.context.runIndex !== undefined) {
			mageDetails.runIndex = error.context.runIndex;
		}

		if (error.context.parameter !== undefined) {
			mageDetails.parameter = error.context.parameter;
		}

		if (error.context.causeDetailed) {
			mageDetails.causeDetailed = error.context.causeDetailed;
		}
	}

	if (error.timestamp) {
		mageDetails.time = new Date(error.timestamp).toLocaleString();
	}

	mageDetails.mageVersion = mageVersion.value;

	mageDetails.binaryDataMode = rootStore.binaryDataMode;

	if (error.cause) {
		mageDetails.cause = error.cause;
	}

	mageDetails.stackTrace = error.stack?.split('\n');

	errorInfo.mageDetails = mageDetails;
	...
	<div class="node-error-view">
		<div class="node-error-view__header">
			...
			<div
				...
				v-mage-html="getErrorDescription()"
			></div>
				...
				<details class="node-error-view__details">
					...
					<div class="node-error-view__details-content">
						...
						<div class="node-error-view__details-row">
							<p class="node-error-view__details-label">
								{{ i18n.baseText('nodeErrorView.details.mageVersion') }}
							</p>
							<p class="node-error-view__details-value">
								<code>{{ mageVersion }}</code>
							</p>
						</div>
