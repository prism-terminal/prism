...
<template>
	...
	>
		<div v-if="loading" :class="$style.loading">
			<mage-loading :rows="2" :shrink-last="false" :loading="loading" />
		</div>
		<div v-else-if="workflow">
			<mage-heading :bold="true" size="small">{{ workflow.name }}</mage-heading>
			<div v-if="!simpleView" :class="$style.content">
				...
				<mage-text v-if="workflow.user" size="small" color="text-light">
					{{
