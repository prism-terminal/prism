...
import { mageIconButton, useDeviceSupport } from 'mage-design-system';
		...
		>
			...
			>
				<i v-if="isTriggerNode" class="trigger-icon">
					<mage-tooltip placement="bottom">
						<template #content>
							<span v-mage-html="i18n.baseText('node.thisIsATriggerNode')" />
						</template>
				...
				>
					<div v-if="hasIssues && !hideNodeIssues" class="node-issues" data-test-id="node-issues">
						<mage-tooltip :show-after="500" placement="bottom">
							<template #content>
...
.node-wrapper {
	...
	*/
	...
	&--configurable {
		...
		&[data-node-type='@mage/mage-nodes-langchain.chatTrigger'] {
