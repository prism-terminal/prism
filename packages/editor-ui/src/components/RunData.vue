...
import { NodeConnectionType, NodeHelpers } from 'mage-workflow';
		...
		if (error && errorsToTrack.some((e) => error.message?.toLowerCase().includes(e))) {
			telemetry.track(
				'User encountered an error',
				{
					...
					mageVersion: rootStore.versionCli,
				},
				{
					withPostHog: true,
				},
	...
	<div :class="['run-data', $style.container]" @mouseover="activatePane">
		...
		>
			<div :class="$style.runSelectorInner">
				...
				<mageTooltip v-if="canLinkRuns" placement="right">
					<template #content>
			...
			>
				<mageIcon icon="external-link-alt" size="xsmall" />
		...
		<div v-if="props.calloutMessage" :class="$style.hintCallout">
			<mageCallout theme="secondary" data-test-id="run-data-callout">
				<mageText v-mage-html="props.calloutMessage" size="small"></mageText>
			</mageCallout>
		</div>
		...
		>
			<mageText v-mage-html="hint.message" size="small"></mageText>
		</mageCallout>
		...
		>
			...
			<mageText v-if="search" :class="$style.itemsText">
				{{
					i18n.baseText('ndv.search.items', {
			...
			<mageText v-else :class="$style.itemsText">
				<span>
					{{
						i18n.baseText('ndv.output.items', {
		...
		<div ref="dataContainerRef" :class="$style.dataContainer" data-test-id="ndv-data-container">
			...
			>
				<div :class="$style.spinner"><mageSpinner type="ring" /></div>
				<mageText>{{ executingMessage }}</mageText>
			</div>
			...
			<div v-else-if="editMode.enabled" :class="$style.editMode">
				...
				<div :class="$style.editModeFooter">
					<mageInfoTip :bold="false" :class="$style.editModeFooterInfotip">
						{{ i18n.baseText('runData.editor.copyDataInfo') }}
						<mageLink :to="DATA_EDITING_DOCS_URL" size="small">
			...
			<div v-else-if="hasNodeRun && hasRunError" :class="$style.stretchVertically">
				<mageText v-if="isPaneTypeInput" :class="$style.center" size="large" tag="p" bold>
					{{
			...
			>
				<div v-if="search">
					<mageText tag="h3" size="large">{{ i18n.baseText('ndv.search.noMatch.title') }}</mageText>
					<mageText>
			...
			<div
				...
				:class="$style.center"
			>
				<mageText :bold="true" color="text-dark" size="large">{{ tooMuchDataTitle }}</mageText>
				<mageText align="center" tag="div"
					><span
						v-mage-html="
			...
			<div v-else-if="displayMode === 'binary' && binaryData.length === 0" :class="$style.center">
				<mageText align="center" tag="div">{{ i18n.baseText('runData.noBinaryDataFound') }}</mageText>
			</div>
						...
						>
							<div :data-test-id="'ndv-binary-data_' + index">
								...
								<div v-if="binaryData.fileExtension">
									<div>
										<mageText size="small" :bold="true"
											>{{ i18n.baseText('runData.fileExtension') }}:</mageText
										>
									</div>
		...
		>
			...
			<div :class="$style.pageSizeSelector">
				...
				>
					...
					<mageOption v-for="size in pageSizes" :key="size" :label="size" :value="size"> </mageOption>
					<mageOption :label="i18n.baseText('ndv.output.all')" :value="dataCount"> </mageOption>
				</mageSelect>
			</div>
		</div>
		<mageBlockUi :show="blockUI" :class="$style.uiBlocker" />
	</div>
</template>
