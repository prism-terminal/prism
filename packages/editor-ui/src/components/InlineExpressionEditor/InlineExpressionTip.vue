...
<template>
	<div :class="[$style.tip, { [$style.drag]: tip === 'drag' }]">
		<mage-text size="small" :class="$style.tipText"
			>{{ i18n.baseText('parameterInput.tip') }}:
		</mage-text>

		<div v-if="tip === 'drag'" :class="$style.content">
			<mage-text size="small" :class="$style.text">
		...
		<div v-else-if="tip === 'dotPrimitive'" :class="$style.content">
			<span v-mage-html="i18n.baseText('expressionTip.typeDotPrimitive')" />
		</div>

		<div v-else-if="tip === 'dotObject'" :class="$style.content">
			<span v-mage-html="i18n.baseText('expressionTip.typeDotObject')" />
		</div>

		<div v-else :class="$style.content">
			<span v-mage-html="i18n.baseText('expressionTip.javascript')" />
		</div>
	</div>
</template>
