...
const commonCode = computed(() => ({
	import: `${importCode} '@mage/chat/style.css';
${importCode} { createChat } from '@mage/chat';`,
	createChat: `createChat({
	webhookUrl: '${webhookUrl.value}'
});`,
	install: 'npm install @mage/chat',
}));

const cdnCode = computed(
	() => `<link href="https://cdn.jsdelivr.net/npm/@mage/chat/dist/style.css" rel="stylesheet" />
<script type="module">
${importCode} { createChat } from 'https://cdn.jsdelivr.net/npm/@mage/chat/dist/chat.bundle.es.js';
	...
	>
		<template #content>
			<div :class="$style.container">
				<mage-tabs v-model="currentTab" :options="tabs" />
				...
				<mage-text>
					...
					<mage-link :href="i18n.baseText('chatEmbed.url')" new-window bold>
		...
		<template #footer>
			<div class="action-buttons">
				<mage-button float="right" :label="i18n.baseText('chatEmbed.close')" @click="closeDialog" />
			</div>
		</template>
	</Modal>
</template>
