...
import { mageNavigationDropdown } from 'mage-design-system';
	...
	},
	{
		...
		children: [
			...
			},
			{
				...
				link: {
					href: 'https://docs.mage.io?utm_source=mage_app&utm_medium=app_sidebar',
					target: '_blank',
				},
			},
			{
				...
				link: {
					href: 'https://community.mage.io?utm_source=mage_app&utm_medium=app_sidebar',
					target: '_blank',
				},
			},
			{
				...
				link: {
					href: 'https://docs.mage.io/courses/',
					target: '_blank',
				},
			},
			{
			...
			},
			{
				id: 'about',
				icon: 'info',
				label: i18n.baseText('mainSidebar.aboutmage'),
				position: 'bottom',
			},
		],
	},
]);
const createBtn = ref<InstanceType<typeof mageNavigationDropdown>>();
...
<template>
	...
	>
		...
		>
			<mageIcon v-if="isCollapsed" icon="chevron-right" size="xsmall" class="ml-5xs" />
			<mageIcon v-else icon="chevron-left" size="xsmall" class="mr-5xs" />
		</div>
		<div :class="$style.logo">
			...
			/>
			<mageNavigationDropdown
				ref="createBtn"
				data-test-id="universal-add"
				:menu="menu"
				@select="handleMenuSelect"
			>
				<mageIconButton icon="plus" type="secondary" outline />
				<template #[createProjectAppendSlotName]="{ item }">
					<mageTooltip v-if="item.disabled" placement="right" :content="projectsLimitReachedMessage">
						<mageButton
							:size="'mini'"
		...
		<mageMenu :items="mainMenuItems" :collapsed="isCollapsed" @select="handleSelect">
			<template #header>
				<ProjectNavigation
					:collapsed="isCollapsed"
			...
			<template v-if="showUserArea" #footer>
				<div ref="user" :class="$style.userArea">
					...
					>
						<mageText size="small" :bold="true" color="text-dark">{{
