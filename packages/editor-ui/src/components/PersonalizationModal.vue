...
import { createEventBus, createFormEventBus } from 'mage-design-system/utils';
...
const survey = computed<IFormInputs>(() => [
	{
		...
		properties: {
			...
			options: [
				...
				{
					label: i18n.baseText('personalizationModal.imNotUsingmageForWork'),
	...
	},
	{
		...
		properties: {
			type: 'select',
			label: i18n.baseText('personalizationModal.howDidYouHearAboutmage'),
			placeholder: i18n.baseText('personalizationModal.select'),
			options: [
				{
					label: 'Google',
...
const onSubmit = async (values: IPersonalizationLatestVersion) => {
	...
	try {
		const completedSurvey: IPersonalizationLatestVersion = {
			...
			personalization_survey_mage_version: rootStore.versionCli,
		};
...
<template>
	<Modal
		...
		:title="i18n.baseText('personalizationModal.customizemage')"
