		...
		<transition name="fade" mode="out-in">
			...
			<div v-else-if="isListeningForEvents" key="listening">
				...
				<div v-if="isWebhookNode">
					<mage-text tag="div" size="large" color="text-dark" class="mb-2xs" bold>{{
						i18n.baseText('ndv.trigger.webhookNode.listening')
					}}</mage-text>
				...
				<div v-else>
					...
					<div v-if="displayChatButton">
						<mage-button class="mb-xl" @click="openWebhookUrl()">
			...
			<div v-else key="default">
				<div v-if="isActivelyPolling" class="mb-xl">
					<mage-spinner type="ring" />
				</div>

				<div :class="$style.action">
					<div :class="$style.header">
						<mage-heading v-if="header" tag="h1" bold>
							{{ header }}
						</mage-heading>
						<mage-text v-if="subheader">
				...
				<mage-text v-if="activationHint" size="small" @click="onLinkClick">
					<span v-mage-html="activationHint"></span>&nbsp;
				</mage-text>
				<mage-link
					v-if="activationHint && executionsHelp"
					size="small"
					@click="expandExecutionHelp"
					>{{ i18n.baseText('ndv.trigger.moreInfo') }}</mage-link
				>
				<mage-info-accordion
					v-if="executionsHelp"
					ref="help"
