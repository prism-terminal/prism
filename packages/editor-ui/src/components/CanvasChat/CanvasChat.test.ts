...
import type { INodeTypeDescription } from 'mage-workflow';
...
import { ChatOptionsSymbol, ChatSymbol } from '@mage/chat/constants';
import { chatEventBus } from '@mage/chat/event-buses';
...
import type { ChatMessage } from '@mage/chat/types';
...
const mockNodeTypes: INodeTypeDescription[] = [
	{
		...
		name: '@mage/mage-nodes-langchain.agent',
		properties: [],
		defaults: {
			name: 'AI Agent',
		},
...
describe('CanvasChat', () => {
	...
	describe('resize functionality', () => {
		...
		it('should persist resize dimensions', () => {
			...
			expect(mockStorage.getItem).toHaveBeenCalledWith('mage_CANVAS_CHAT_HEIGHT');
			expect(mockStorage.getItem).toHaveBeenCalledWith('mage_CANVAS_CHAT_WIDTH');
		});
	});
