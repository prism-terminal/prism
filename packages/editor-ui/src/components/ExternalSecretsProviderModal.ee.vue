...
import type { IParameterLabel } from 'mage-workflow';
		...
		<template #content>
			<div v-if="provider" :class="$style.container">
				...
				<div v-if="connectionState !== 'initializing'" class="mb-l">
					...
					>
						...
						<span v-if="provider.connected">
							...
							<mage-link :href="i18n.baseText('settings.externalSecrets.docs.use')" size="small">
								{{
									i18n.baseText(
					...
					<mage-callout v-else-if="connectionState === 'error'" theme="danger">
						{{
							i18n.baseText(
				...
				<form
					...
					@submit.prevent
				>
					<mage-notice v-if="property.type === 'notice'" :content="property.displayName" />
