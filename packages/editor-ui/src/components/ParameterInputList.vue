...
import { deepCopy, ADD_FORM_NOTICE } from 'mage-workflow';
...
import { mageNotice, mageIconButton, mageInputLabel, mageText, mageIcon } from 'mage-design-system';
		...
		>
			...
			>
				...
				<Suspense v-if="!asyncLoadingError">
					...
					<template #fallback>
						<mageText size="small" class="async-notice">
							<mageIcon icon="sync-alt" size="xsmall" :spin="true" />
							{{ i18n.baseText('parameterInputList.loadingFields') }}
						</mageText>
					</template>
				</Suspense>
				<mageText v-else size="small" color="danger" class="async-notice">
