...
import { CREDENTIAL_EMPTY_VALUE, NodeHelpers } from 'mage-workflow';
...
import { mageIcon, mageInput, mageInputNumber, mageOption, mageSelect } from 'mage-design-system';
...
const inputField = ref<InstanceType<typeof mageInput | typeof mageSelect> | HTMLElement>();
...
function trackExpressionEditOpen() {
	...
	if (node.value.type.startsWith('mage-nodes-base') || isCredentialOnlyNodeType(node.value.type)) {
...
onMounted(() => {
	...
	void externalHooks.run('parameterInput.mount', {
		...
		inputFieldRef: inputField.value as InstanceType<typeof mageInput>,
	});
});
				...
				>
					<div class="list-option">
						...
						<div
							...
							v-mage-html="getOptionsOptionDescription(option)"
						></div>
					</div>
				</mageOption>
			</mageSelect>
