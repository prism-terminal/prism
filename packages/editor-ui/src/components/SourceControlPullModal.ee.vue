	...
	>
		<template #content>
			<div :class="$style.container">
				<mage-text>
					...
					<mage-link :to="i18n.baseText('settings.sourceControl.docs.using.pushPull.url')">
				...
				<div v-if="modifiedWorkflowFiles.length > 0" class="mt-l">
					<ul :class="$style.filesList">
						<li v-for="file in modifiedWorkflowFiles" :key="file.id">
							<mage-link :class="$style.fileLink" new-window :to="`/workflow/${file.id}`">
								{{ file.name }}
								<mage-icon icon="external-link-alt" />
							</mage-link>
						</li>
					</ul>
				</div>
		...
		<template #footer>
			<div :class="$style.footer">
				<mage-button type="tertiary" class="mr-2xs" @click="close">
					{{ i18n.baseText('settings.sourceControl.modals.pull.buttons.cancel') }}
				</mage-button>
				<mage-button type="primary" @click="pullWorkfolder">
