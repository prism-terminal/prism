...
import type { ITelemetryTrackProperties } from 'mage-workflow';
...
<template>
	...
	>
			...
			<div v-else :class="$style.container">
				...
				<enterprise-edition :features="[EnterpriseEditionFeature.Sharing]" :class="$style.content">
					<div>
						...
						/>
						<mage-info-tip v-if="isHomeTeamProject" :bold="false" class="mt-s">
		...
		<template #footer>
			...
			>
				<mage-text v-show="isDirty" color="text-light" size="small" class="mr-xs">
					{{ i18n.baseText('workflows.shareModal.changesHint') }}
				</mage-text>
				<mage-button v-if="isHomeTeamProject" type="secondary" @click="modalBus.emit('close')">
