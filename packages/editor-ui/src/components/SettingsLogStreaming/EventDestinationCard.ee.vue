...
import type { MessageEventBusDestinationOptions } from 'mage-workflow';
import { deepCopy, defaultMessageEventBusDestinationOptions } from 'mage-workflow';
import type { BaseTextKey } from '@/plugins/i18n';
import type { EventBus } from 'mage-design-system';
...
<template>
	<mage-card :class="$style.cardLink" data-test-id="destination-card" @click="onClick">
		<template #header>
			<div>
				<mage-heading tag="h2" bold :class="$style.cardHeading">
					{{ destination.label }}
				</mage-heading>
				<div :class="$style.cardDescription">
					<mage-text color="text-light" size="small">
		...
		<template #append>
			<div ref="cardActions" :class="$style.cardActions">
				<div :class="$style.activeStatusText" data-test-id="destination-activator-status">
					<mage-text v-if="nodeParameters.enabled" :color="'success'" size="small" bold>
						{{ i18n.baseText('workflowActivator.active') }}
					</mage-text>
					<mage-text v-else color="text-base" size="small" bold>
				...
				<mage-action-toggle :actions="actions" theme="dark" @action="onAction" />
			</div>
		</template>
	</mage-card>
</template>
