...
const TEST_COLLECTION = {
	...
	nodes: [
		...
		},
		{
			id: 1121,
			icon: 'fa:link',
			name: '@mage/mage-nodes-langchain.chainSummarization',
			iconData: {
				icon: 'link',
				type: 'icon',
			},
		...
		},
		{
			id: 1123,
			icon: 'fa:link',
			name: '@mage/mage-nodes-langchain.chainLlm',
			iconData: {
				icon: 'link',
				type: 'icon',
			},
...
describe('TemplatesInfoCard', () => {
	...
	it('should render component properly', () => {
		...
		expect(container.querySelectorAll('.mage-node-icon').length).toBe(TEST_COLLECTION.nodes.length);
	});
