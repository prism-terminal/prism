...
import type { Rule, RuleGroup } from 'mage-design-system/types';
...
<template>
	<div>
		<mage-heading size="2xlarge" tag="h1">{{
		...
		>
			<mage-callout theme="secondary" icon="info-circle" class="mt-2xl mb-l">
			...
			<mage-heading size="xlarge" tag="h2" class="mb-s">{{
			...
			<div :class="$style.group">
				...
				<div :class="$style.groupFlex">
					...
					<mage-button
						...
						@click="onDisconnect"
						>{{ locale.baseText('settings.sourceControl.button.disconnect') }}</mage-button
					>
				</div>
			</div>
			<div v-if="sourceControlStore.preferences.publicKey" :class="$style.group">
				...
				<mage-notice type="info" class="mt-s">
			...
			<mage-button
				...
				@click="onConnect"
				>{{ locale.baseText('settings.sourceControl.button.connect') }}</mage-button
			>
			<div v-if="isConnected" data-test-id="source-control-connected-content">
				...
				<div :class="$style.group">
					...
					<div>
						<mage-color-picker v-model="sourceControlStore.preferences.branchColor" size="small" />
					</div>
				</div>
				<div :class="[$style.group, 'pt-s']">
					<mage-button
						...
						@click="onSave"
						>{{ locale.baseText('settings.sourceControl.button.save') }}</mage-button
					>
				</div>
			</div>
		</div>
