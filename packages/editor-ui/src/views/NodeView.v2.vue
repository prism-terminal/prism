...
import { TelemetryHelpers, NodeConnectionType, jsonParse } from 'mage-workflow';
import type { IDataObject, ExecutionSummary, IConnection, IWorkflowBase } from 'mage-workflow';
...
function emitPostMessageReady() {
	if (window.parent) {
		window.parent.postMessage(
			JSON.stringify({ command: 'mageReady', version: rootStore.versionCli }),
			'*',
		);
	}
}
