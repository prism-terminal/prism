		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<mage-heading v-if="collection && collection.name" tag="h1" size="2xlarge">
						{{ collection.name }}
					</mage-heading>
					<mage-text v-if="collection && collection.name" color="text-base" size="small">
						{{ i18n.baseText('templates.collection') }}
					</mage-text>
					<mage-loading :loading="!collection || !collection.name" :rows="2" variant="h1" />
				</div>
			</div>
			<div v-else :class="$style.notFound">
				<mage-text color="text-base">{{ i18n.baseText('templates.collectionsNotFound') }}</mage-text>
			</div>
		</template>
