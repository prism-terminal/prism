...
import mageHeading from 'mage-design-system/components/mageHeading';
import mageLink from 'mage-design-system/components/mageLink';
...
<template>
	<TemplatesView :go-back-enabled="true">
		<template #header>
			<mageHeading v-if="isReady" tag="h1" size="2xlarge"
				>{{ i18n.baseText('templateSetup.title', { interpolate: { name: title } }) }}
			</mageHeading>
			<mage-loading v-else variant="h1" />
		</template>
			...
			<div :class="$style.grid">
				...
				<div>
					...
					<div v-else :class="$style.appCredentialsContainer">
						<mage-loading :class="$style.appCredential" variant="p" :rows="3" />
						<mage-loading :class="$style.appCredential" variant="p" :rows="3" />
					</div>
				</div>

				<div :class="$style.actions">
					<mageLink :href="skipSetupUrl" :new-window="false" @click="onSkipSetup($event)">{{
