		...
		<template #header>
			<div v-if="!notFoundError" :class="$style.wrapper">
				<div :class="$style.title">
					<mage-heading v-if="template && template.name" tag="h1" size="2xlarge">{{
						template.name
					}}</mage-heading>
					<mage-text v-if="template && template.name" color="text-base" size="small">
						{{ i18n.baseText('generic.workflow') }}
					</mage-text>
					<mage-loading :loading="!template || !template.name" :rows="2" variant="h1" />
				</div>
				<div :class="$style.button">
					...
					/>
					<mage-loading :loading="!template" :rows="1" variant="button" />
				</div>
			</div>
			<div v-else :class="$style.notFound">
				<mage-text color="text-base">{{ i18n.baseText('templates.workflowsNotFound') }}</mage-text>
			</div>
		</template>
