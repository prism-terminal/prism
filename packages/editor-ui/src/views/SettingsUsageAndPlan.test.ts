...
describe('SettingsUsageAndPlan', () => {
	beforeEach(() => {
		...
		usageStore.viewPlansUrl = 'https://subscription.mage.io';
		usageStore.managePlanUrl = 'https://subscription.mage.io';
	});
	...
	it('should not show badge but unlock notice', async () => {
		...
		expect(container.querySelector('.mage-badge')).toBeNull();
	...
	it('should show community registered badge', async () => {
		...
		expect(getByRole('heading', { level: 3 })).toContain(container.querySelector('.mage-badge'));
		expect(container.querySelector('.mage-badge')).toHaveTextContent('Registered');
	});
});
