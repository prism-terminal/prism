...
function onUpgrade() {
	void goToUpgrade('settings-mage-api', 'upgrade-api', 'redirect');
}
...
<template>
	<div :class="$style.container">
		...
		<div v-if="apiKeys.length">
				...
				<mage-info-tip :bold="false">
					<i18n-t keypath="settings.api.view.info" tag="span">
						...
						<template #webhookAction>
							<a
								href="https://docs.mage.io/integrations/core-nodes/mage-nodes-base.webhook/"
								target="_blank"
			...
			<mage-card class="mb-4xs" :class="$style.card">
				<span :class="$style.delete">
					<mage-link :bold="true" @click="showDeleteModal">
			...
			<div :class="$style.hint">
				<mage-text size="small">
					{{ i18n.baseText(`settings.api.view.${isSwaggerUIEnabled ? 'tryapi' : 'more-details'}`) }}
				</mage-text>
				{{ ' ' }}
				<mage-link :to="apiDocsURL" :new-window="true" size="small">
					{{
						i18n.baseText(
