...
import type { magePlusEndpoint } from '@/plugins/jsplumb/magePlusEndpointType';
import {
	magePlusEndpointType,
	EVENT_PLUS_ENDPOINT_CLICK,
} from '@/plugins/jsplumb/magePlusEndpointType';
import type { mageAddInputEndpoint } from '@/plugins/jsplumb/mageAddInputEndpointType';
import {
	EVENT_ADD_INPUT_ENDPOINT_CLICK,
	mageAddInputEndpointType,
} from '@/plugins/jsplumb/mageAddInputEndpointType';
...
import { useDeviceSupport } from 'mage-design-system';
...
export default defineComponent({
			...
			try {
				...
				if (window.parent) {
					window.parent.postMessage(
						JSON.stringify({ command: 'mageReady', version: this.rootStore.versionCli }),
						'*',
					);
				}
			} catch (error) {
	...
	methods: {
		...
		resetEndpointsErrors() {
			...
			allEndpoints
				.filter((endpoint) => endpoint?.endpoint.type === mageAddInputEndpointType)
				.forEach((endpoint) => {
					const mageAddInputEndpoint = endpoint?.endpoint as mageAddInputEndpoint;
					if (mageAddInputEndpoint && (endpoint?.connections ?? []).length > 0) {
						mageAddInputEndpoint.resetError();
					}
				});
		},
			...
			nodeIssues.forEach((node) => {
				...
				Object.keys(node?.issues?.input ?? {}).forEach((connectionType) => {
					...
					inputEndpointsWithIssues.forEach((endpoint) => {
						const mageAddInputEndpoint = endpoint?.endpoint as mageAddInputEndpoint;
						if (mageAddInputEndpoint) {
							mageAddInputEndpoint.setError();
						}
					});
				});
			});
		...
		onConnectionDragAbortDetached() {
			Object.values(this.instance?.endpointsByElement)
				...
				.filter((endpoint) => endpoint.endpoint.type === 'magePlus')
			...
			if (this.instance) {
				...
				for (const element of Object.values(elements)) {
					...
					for (const endpoint of endpoints || []) {
						...
						if (endpointInstance && endpointInstance.type === magePlusEndpointType) {
							(endpointInstance as magePlusEndpoint).unbindEvents();
						}
					}
				}
			}
		...
		onNodeRun({
			...
			if (data === null || data.length === 0 || waiting) {
				...
				endpoints.forEach((endpoint: Endpoint) => {
					if (endpoint.endpoint.type === 'magePlus') {
						(endpoint.endpoint as magePlusEndpoint).clearSuccessOutput();
					}
				});
