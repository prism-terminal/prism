...
import mageInfoTip from 'mage-design-system/components/mageInfoTip';
...
<template>
	<div class="settings-usage-and-plan">
		<mage-heading tag="h2" size="2xlarge">{{
			locale.baseText('settings.usageAndPlan.title')
		}}</mage-heading>
		<div v-if="!usageStore.isLoading">
			<mage-heading tag="h3" :class="$style.title" size="large">
				...
				<span v-if="badgedPlanName.badge && badgedPlanName.name" :class="$style.titleTooltip">
					<mageTooltip placement="top">
						...
						<mageBadge>{{ badgedPlanName.badge }}</mageBadge>
					</mageTooltip>
				</span>
			</mage-heading>

			<mageNotice v-if="isCommunity && canUserRegisterCommunityPlus" class="mt-0" theme="warning">
			...
			<div :class="$style.quota">
				<mage-text size="medium" color="text-light">
			...
			<mageInfoTip>{{ locale.baseText('settings.usageAndPlan.activeWorkflows.hint') }}</mageInfoTip>
			...
			<div :class="$style.buttons">
				...
				<mage-button v-if="usageStore.managementToken" size="large" @click="onManagePlan">
					<a :href="managePlanUrl" target="_blank">{{
						locale.baseText('settings.usageAndPlan.button.manage')
					}}</a>
				</mage-button>
				<mage-button v-else size="large" @click.prevent="onViewPlans">
			...
			>
				...
				<template #footer>
					<mage-button type="secondary" @click="activationKeyModal = false">
						{{ locale.baseText('settings.usageAndPlan.dialog.activation.cancel') }}
					</mage-button>
					<mage-button @click="onLicenseActivation">
