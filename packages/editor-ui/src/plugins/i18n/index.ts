...
import { locale } from 'mage-design-system';
import type { INodeProperties, INodePropertyCollection, INodePropertyOptions } from 'mage-workflow';
...
export class I18nClass {
	...
	shortNodeType(longNodeType: string) {
		return longNodeType.replace('mage-nodes-base.', '');
	}
	...
	credText() {
		...
		const credentialPrefix = `mage-nodes-base.credentials.${credentialType}`;
		const context = this;
	...
	nodeText() {
		...
		const initialKey = `mage-nodes-base.nodes.${nodeType}.nodeView`;
		const context = this;
		...
		return {
			...
			eventTriggerDescription(nodeType: string, eventTriggerDescription: string) {
				return context.dynamicRender({
					key: `mage-nodes-base.nodes.${nodeType}.nodeView.eventTriggerDescription`,
...
async function setLanguage(language: string) {
	...
	// update mage design system and element ui
