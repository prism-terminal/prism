...
mage allows for internalization of the majority of UI text:
...
Currently, mage does _not_ allow for internalization of:
...
By default, mage runs in the `en` (English) locale. To have run it in a different locale, set the `mage_DEFAULT_LOCALE` environment variable to a locale identifier. When running in a non-`en` locale, mage will display UI strings for the selected locale and fall back to `en` for any untranslated strings.

```
export mage_DEFAULT_LOCALE=de
pnpm start
```
...
```
Initializing mage process
mage ready on 0.0.0.0, port 5678
Version: 0.156.0
Locale: de
...
```json
{
	...
	"nodeView.owner.placeholder": "🇩🇪 mage-io",
...
```json
{
	...
	"nodeView.additionalParameters.options.author.values.email.placeholder": "🇩🇪 jan@mage.io"
}
```
...
pnpm mage-generate-translations
pnpm watch
```
...
If a `headerText` section was changed, re-run `pnpm mage-generate-translations` in `/nodes-base`.
