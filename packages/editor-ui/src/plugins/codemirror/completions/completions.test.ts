...
import { mageLang } from '@/plugins/codemirror/mageLang';
...
describe('Resolution-based completions', () => {
	...
	describe('recommended completions', () => {
		...
		test('should recommend extractEmail() for: {{ "string with test@mage.io in it".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce(
				'string with test@mage.io in it',
			);
			const options = completions('{{ "string with test@mage.io in it".| }}');
			expect(options?.[0]).toEqual(
				expect.objectContaining({ label: 'extractEmail()', section: RECOMMENDED_SECTION }),
			);
		});

		test('should recommend extractDomain(), isEmail() for: {{ "test@mage.io".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce('test@mage.io');
			const options = completions('{{ "test@mage.io".| }}');
			expect(options?.[0]).toEqual(
		...
		test('should recommend extractDomain(), extractUrlPath() for: {{ "https://mage.io/pricing".| }}', () => {
			vi.spyOn(workflowHelpers, 'resolveParameter').mockReturnValueOnce('https://mage.io/pricing');
			const options = completions('{{ "https://mage.io/pricing".| }}');
			expect(options?.[0]).toEqual(
				expect.objectContaining({ label: 'extractDomain()', section: RECOMMENDED_SECTION }),
			);
			expect(options?.[1]).toEqual(
