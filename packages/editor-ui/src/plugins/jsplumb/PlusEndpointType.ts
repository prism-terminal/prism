...
export type ComputedmagePlusEndpoint = [number, number, number, number, number];
export type mageEndpointLabelLength = 'small' | 'medium' | 'large';
interface magePlusEndpointParams extends EndpointRepresentationParams {
	dimensions: number;
...
export const magePlusEndpointType = 'magePlus';
export const EVENT_PLUS_ENDPOINT_CLICK = 'eventPlusEndpointClick';
export class magePlusEndpoint extends EndpointRepresentation<ComputedmagePlusEndpoint> {
	params: magePlusEndpointParams;
	...
	constructor(endpoint: Endpoint, params: magePlusEndpointParams) {
	...
	static type = magePlusEndpointType;

	type = magePlusEndpoint.type;
...
export const magePlusEndpointHandler: EndpointHandler<magePlusEndpoint, ComputedmagePlusEndpoint> = {
	type: magePlusEndpoint.type,
	cls: magePlusEndpoint,
	compute: (
		ep: EndpointRepresentation<ComputedmagePlusEndpoint>,
		anchorPoint: AnchorPlacement,
	): ComputedmagePlusEndpoint => {
		if (!(ep instanceof magePlusEndpoint)) {
	...
	getParams: (ep: magePlusEndpoint): magePlusEndpointParams => {
		return ep.params;
	},
};
