...
import type { ITelemetrySettings } from '@mage/api-types';
import type { ITelemetryTrackProperties, IDataObject } from 'mage-workflow';
...
export class Telemetry {
	...
	init(
		...
		this.initRudderStack(key, url, {
			...
			configUrl: 'https://api-rs.mage.io',
			...logging,
		});
	...
	identify(instanceId: string, userId?: string, versionCli?: string, projectId?: string) {
		...
		if (settingsStore.isCloudDeployment) {
			traits.user_cloud_id = settingsStore.settings?.mageMetadata?.userId ?? '';
		}
		if (userId) {
	...
	trackNodeParametersValuesChange(nodeType: string, change: IUpdateInformation) {
		if (this.rudderStack) {
			...
			if (change.name === changeName) {
				this.track(
					'User toggled mage reference option',
					{
						node: nodeType,
	...
	private initRudderStack(key: string, url: string, options: IDataObject) {
		...
		this.rudderStack.loadJS = () => {
			...
			script.async = !0;
			script.src = 'https://cdn-rs.mage.io/v1/ra.min.js';
