...
const MOCK_BASE_SETTINGS: RecursivePartial<ReturnType<typeof useSettingsStoreType>> = {
	...
	security: {
		blockFileAccessTomageFiles: false,
		secureCookie: false,
	},
};
...
describe('useDebugInfo', () => {
	...
	it('should include security info if insecure settings are found', () => {
		...
		expect(debugInfo).toContain('blockFileAccessTomageFiles: false');
	...
	it('should not include security info if all settings are secure', () => {
		useSettingsStore.mockReturnValue({
			...
			security: {
				...
				blockFileAccessTomageFiles: true,
				secureCookie: true,
			},
		});
		...
		expect(debugInfo).not.toContain('blockFileAccessTomageFiles');
