...
import { mage_PRICING_PAGE_URL } from '@/constants';
...
export function usePageRedirectionHelper() {
	 ...
	 * cloud dashboard that redirects the user to the /manage page where they can upgrade to a new mage version.
	...
	const generateUpgradeLink = async (source: string, utm_campaign: string) => {
		let upgradeLink = mage_PRICING_PAGE_URL;
