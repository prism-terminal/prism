...
import { NodeHelpers, ExpressionEvaluatorProxy, NodeConnectionType } from 'mage-workflow';
import type {
	INodeProperties,
...
export function useNodeHelpers() {
					...
					(targetInputIndex: string) => {
						...
						if (endpoint?.endpoint) {
							...
							if (output && output.total > 0) {
								(endpoint.endpoint as magePlusEndpoint).setSuccessOutput(
	...
	const SINGLE_EXECUTION_NODES: { [key: string]: { [key: string]: NodeParameterValue[] } } = {
		'mage-nodes-base.code': {
			mode: [undefined, 'runOnceForAllItems'],
		},
		'mage-nodes-base.executeWorkflow': {
			mode: [undefined, 'once'],
		},
		'mage-nodes-base.crateDb': {
			operation: [undefined, 'update'], // default insert
		},
		'mage-nodes-base.timescaleDb': {
			operation: [undefined, 'update'], // default insert
		},
		'mage-nodes-base.microsoftSql': {
			operation: [undefined, 'update', 'delete'], // default insert
		},
		'mage-nodes-base.questDb': {
			operation: [undefined], // default insert
		},
		'mage-nodes-base.mongoDb': {
			operation: ['insert', 'update'],
		},
		'mage-nodes-base.redis': {
