...
import { NodeConnectionType, STICKY_NODE_TYPE, type INodeTypeDescription } from 'mage-workflow';
	...
	describe('canPinData()', () => {
		...
		it('does not allow pin without output', async () => {
			const node = ref({
				...
				typeVersion: 1,
				type: 'mage-nodes-base.stopAndError',
			} as INodeUi);
			getNodeType.mockReturnValue(makeNodeType([], 'mage-nodes-base.stopAndError'));
