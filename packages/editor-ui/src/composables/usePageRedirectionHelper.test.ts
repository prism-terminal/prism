...
describe('usePageRedirectionHelper', () => {
	...
	beforeEach(() => {
		...
		const url = 'https://test.app.mage.cloud';
		...
		versionStore.setVersionNotificationSettings({
			...
			endpoint: '',
			infoUrl:
				'https://docs.mage.io/release-notes/#mage1652?utm_source=mage_app&utm_medium=instance_upgrade_releases',
		});
	});
	...
	test.each([
		[
			...
			'production',
			ROLE.Owner,
			'https://mage.io/pricing?utm_campaign=upgrade-api&source=advanced-permissions',
		],
		[
			'default',
			'development',
		...
		[
			...
			'production',
			ROLE.Owner,
			`https://app.mage.cloud/login?code=123&returnPath=${encodeURIComponent(
				'/account/change-plan',
	...
	test.each([
		[
			...
			'production',
			ROLE.Owner,
			`https://app.mage.cloud/login?code=123&returnPath=${encodeURIComponent('/manage')}`,
		],
		[
			'cloud',
			'production',
	...
	test.each([
		[
			...
			'production',
			ROLE.Owner,
			`https://app.mage.cloud/login?code=123&returnPath=${encodeURIComponent('/dashboard')}`,
		],
		['cloud', 'production', ROLE.Member, 'https://test.app.mage.cloud'],
	])(
