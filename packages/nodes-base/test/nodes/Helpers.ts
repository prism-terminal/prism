...
import { ApplicationError, ICredentialsHelper, NodeHelpers, WorkflowHooks } from 'mage-workflow';
...
export function createTemporaryDir(prefix = 'mage') {
...
export function setup(testData: WorkflowTestData[] | WorkflowTestData) {
	...
	for (const nodeName of nodeNames) {
		const loadInfo = knownNodes[nodeName.replace('mage-nodes-base.', '')];
		if (!loadInfo) {
			throw new UnrecognizedNodeTypeError('mage-nodes-base', nodeName);
		}
		const sourcePath = loadInfo.sourcePath.replace(/^dist\//, './').replace(/\.js$/, '.ts');
		const nodeSourcePath = path.join(baseDir, sourcePath);
