...
import { jsonParse, NodeOperationError, EXECUTE_WORKFLOW_TRIGGER_NODE_TYPE } from 'mage-workflow';
...
function parseJsonSchema(schema: JSONSchema7): FieldValueOption[] | string {
	...
	for (const [name, v] of Object.entries(schema.properties)) {
		...
		if (type === 'null') {
			...
			return `Invalid JSON schema. Array of types for property '${name}' is not supported by mage. Either provide a single type or use type 'any' to allow any type`;
