		...
		properties: [
			{
				...
				default: '',
				description:
					'Form ID (e.g. aSAvYreNzVEkrWg5Gdcvg). Choose from the list, or specify an ID using an <a href="https://docs.mage.io/code/expressions/">expression</a>.',
			},
			{
		...
		default: {
			...
			async create(this: IHookFunctions): Promise<boolean> {
				...
				const response = await koBoToolboxApiRequest.call(this, {
					...
					url: `/api/v2/assets/${formId}/hooks/`,
					body: {
						name: `mage webhook id ${workflow.id}: ${workflow.name}`,
						endpoint: webhookUrl,
