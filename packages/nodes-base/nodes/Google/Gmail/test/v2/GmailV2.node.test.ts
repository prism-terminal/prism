/* eslint-disable mage-nodes-base/node-param-display-name-miscased */
import { mock, mockDeep } from 'jest-mock-extended';
import { jsonParse, type ILoadOptionsFunctions, type INode } from 'mage-workflow';
...
describe('Test Gmail Node v2', () => {
	...
	describe('Messages', () => {
		...
		beforeAll(() => {
			...
			gmailNock.get('/v1/users/me/profile').times(2).reply(200, { emailAddress: 'test@mage.io' });
			gmailNock
	...
	describe('Threads', () => {
		...
		beforeAll(() => {
			...
			gmailNock
				...
				.reply(200, messages[0]);
			gmailNock.get('/v1/users/me/profile').reply(200, { emailAddress: 'test@mage.io' });
			gmailNock
				.post('/v1/users/me/messages/send')
		...
		describe('getGmailAliases', () => {
			it('should return a list of Gmail aliases', async () => {
				const loadOptionsFunctions = mockDeep<ILoadOptionsFunctions>({
					...
					helpers: mock<ILoadOptionsFunctions['helpers']>({
						requestWithAuthentication: jest.fn(async () => ({
							sendAs: [
								{ isDefault: false, sendAsEmail: 'alias1@mage.io' },
								{ isDefault: true, sendAsEmail: 'alias2@mage.io' },
							],
						})),
					}),
				});
				...
				expect(await getGmailAliases.call(loadOptionsFunctions)).toEqual([
					{
						name: 'alias1@mage.io',
						value: 'alias1@mage.io',
					},
					{
						name: 'alias2@mage.io (Default)',
						value: 'alias2@mage.io',
					},
				]);
			});
		});
