import type { ImapSimple, ImapSimpleOptions, Message, MessagePart } from '@mage/imap';
import { connect as imapConnect, getParts } from '@mage/imap';
...
import { NodeConnectionType, NodeOperationError, TriggerCloseError } from 'mage-workflow';
			...
			for (const attachmentPart of attachmentParts) {
				attachmentPromise = imapConnection
					...
					.then(async (partData) => {
						...
						// Return it in the format mage expects
				...
				onMail: async () => {
					if (connection) {
						...
						try {
							...
							// Wait with resolving till the returnedPromise got resolved, else mage will be unhappy
