...
export const personAdditionalFieldsOptions: INodeProperties['options'] = [
	...
	},
	{
		...
		type: 'options', // Action Network accepts a `string[]` of language codes, but supports only one language per person - sending an array of 2+ languages will result in the first valid language being set as the preferred language for the person. Therefore, the user may select only one option in the mage UI.
		default: [],
