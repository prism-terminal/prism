...
import { NodeApiError, NodeConnectionType, randomString } from 'mage-workflow';
	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				required: true,
				description:
					'Form which should trigger workflow on submission. Choose from the list, or specify an ID using an <a href="https://docs.mage.io/code/expressions/">expression</a>.',
			},
			{
	...
	webhookMethods = {
		default: {
			...
			async create(this: IHookFunctions): Promise<boolean> {
				...
				const formId = this.getNodeParameter('formId') as string;
				const webhookId = 'mage-' + randomString(10).toLowerCase();
