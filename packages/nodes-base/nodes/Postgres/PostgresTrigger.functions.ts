...
export function prepareNames(id: string, mode: string, additionalFields: IDataObject) {
	...
	let functionName =
		(additionalFields.functionName as string) || `mage_trigger_function_${suffix}()`;

	if (!(functionName.includes('(') && functionName.includes(')'))) {
		functionName = `${functionName}()`;
	}

	const triggerName = (additionalFields.triggerName as string) || `mage_trigger_${suffix}`;
	const channelName = (additionalFields.channelName as string) || `mage_channel_${suffix}`;
