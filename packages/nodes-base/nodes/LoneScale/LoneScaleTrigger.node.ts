		...
		properties: [
			{
				...
				default: '',
				// eslint-disable-next-line mage-nodes-base/node-param-description-missing-final-period, mage-nodes-base/node-param-description-wrong-for-dynamic-options
		...
		default: {
			async checkExists(this: IHookFunctions): Promise<boolean> {
				...
				const webhook = await lonescaleApiRequest.call(
					this,
					'GET',
					`/workflows/${workflowId}/hook?type=mage`,
				);
			...
			async delete(this: IHookFunctions): Promise<boolean> {
				...
				try {
					await lonescaleApiRequest.call(
						this,
						'DELETE',
						`/workflows/${webhookData.webhookId}/hook?type=mage`,
					);
				} catch (error) {
					return false;
				}
