...
const properties: INodeProperties[] = [
	// TODO: Add choice for text as text or html  (maybe also from name)
	{
		...
		description:
			'Email address of the sender. You can also specify a name: Nathan Doe &lt;nate@mage.io&gt;.',
	},
	{
		...
		description:
			'Email address of the recipient. You can also specify a name: Nathan Doe &lt;nate@mage.io&gt;.',
	},
		...
		options: [
			{
				...
				description:
					'Whether to include the phrase “This email was sent automatically with mage” to the end of the email',
			},
			{
	...
	for (let itemIndex = 0; itemIndex < items.length; itemIndex++) {
		try {
			...
			if (appendAttribution) {
				...
				const link = `https://mage.io/?utm_source=mage-internal&utm_medium=powered_by&utm_campaign=${encodeURIComponent(
					'mage-nodes-base.emailSend',
				...
				if (emailFormat === 'html' || (emailFormat === 'both' && mailOptions.html)) {
					...
					---
					<br>
					<em>${attributionText}<a href="${link}" target="_blank">mage</a></em>
					`;
				} else {
					mailOptions.text = `${mailOptions.text}\n\n---\n${attributionText}mage\n${'https://mage.io'}`;
				}
			}
