...
describe('Workflow templates', () => {
	...
	describe('For api.mage.io', () => {
		beforeEach(() => {
			mockTemplateHost('https://api.mage.io/api/');
		});
		...
		it('Opens website when clicking templates sidebar link', () => {
			...
			mainSidebar.getters
				...
				.and('include', 'https://mage.io/workflows');
			// Link should contain instance address and mage version
			mainSidebar.getters
				.templates()
				.parent('a')
				.then(($a) => {
					...
					expect(params.get('utm_mage_version')).to.match(/[0-9]+\.[0-9]+\.[0-9]+/);
	...
	describe('For a custom template host', () => {
		...
		it('should save template id with the workflow', () => {
			...
			cy.visit(templatesPage.url);
			cy.get('.el-skeleton.mage-loading').should('not.exist');
