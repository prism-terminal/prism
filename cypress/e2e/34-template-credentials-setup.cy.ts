...
describe('Template credentials setup', () => {
	beforeEach(() => {
		cy.intercept(
			'GET',
			`https://api.mage.io/api/templates/workflows/${testTemplate.id}`,
			testTemplate.data,
		).as('getTemplatePreview');
		cy.intercept(
			'GET',
			`https://api.mage.io/api/workflows/templates/${testTemplate.id}`,
			testTemplate.data.workflow,
		).as('getTemplate');
		cy.overrideSettings({
			templates: { enabled: true, host: 'https://api.mage.io/api/' },
		});
	});
	...
	it('should work with a template that has no credentials (ADO-1603)', () => {
		...
		cy.intercept('GET', `https://api.mage.io/api/templates/workflows/${id}`, data);
