...
import CustomNodeWithmageCredentialFixture from '../fixtures/Custom_node_mage_credential.json';
...
describe('Community and custom nodes in canvas', () => {
	beforeEach(() => {
		cy.intercept('/types/nodes.json', { middleware: true }, (req) => {
			...
			req.on('response', (res) => {
				...
				nodes.push(
					CustomNodeFixture,
					CustomNodeWithmageCredentialFixture,
	...
	it('should render custom node with mage credential', () => {
		workflowPage.actions.addNodeToCanvas('Manual');
		workflowPage.actions.addNodeToCanvas('E2E Node with native mage credential', true, true);
...
describe('Community nodes', () => {
	const mockPackage = {
		...
		packageName: 'mage-nodes-chatwork',
		...
		installedNodes: [
			{
				name: 'Chatwork',
				type: 'mage-nodes-chatwork.chatwork',
				latestVersion: 1,
			},
		],
	...
	it('can install, update and uninstall community nodes', () => {
		cy.intercept(
			{
				...
				pathname: '/v2/search',
				query: { q: 'keywords:mage-community-node-package' },
			},
			{ body: {} },
		);
		cy.intercept(
		...
		installFirstCommunityNode('mage-nodes-chatwork@1.0.0');
		...
		cy.intercept(
			{
				...
				query: { name: 'mage-nodes-chatwork' },
			},
			{ statusCode: 204 },
		).as('uninstallPackage');
