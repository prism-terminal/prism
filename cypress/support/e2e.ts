...
beforeEach(() => {
	...
	cy.window().then((win): void => {
		win.localStorage.setItem('mage_THEME', 'light');
		win.localStorage.setItem('mage_AUTOCOMPLETE_ONBOARDED', 'true');
		win.localStorage.setItem('mage_MAPPING_ONBOARDED', 'true');
	});
	...
	cy.intercept({ pathname: '/api/versions/*' }, [
		{
			...
			documentationUrl: 'https://docs.mage.io/release-notes/#mage131',
			nodes: [],
		...
		},
		{
			...
			documentationUrl: 'https://docs.mage.io/release-notes/#mage104',
			nodes: [],
